...
async function dispatch({ messages, opts = {}, hints = {} }) {
    ...
    for (const { provider, model } of candidates) {
        ...
        try {
            ...
            // Empty 200 (no content, no tool calls) = useless answer. Treat it as a
            // failure so we fail over to the next provider instead of returning blank.
            const hasContent = resp && typeof resp.content === 'string' && resp.content.trim().length > 0;
            const hasTools = resp && Array.isArray(resp.toolCalls) && resp.toolCalls.length > 0;
            ...
            metrics.record(provider.name, { ok: false, latencyMs, model: model.id, source: hints.source, err: e.message });
